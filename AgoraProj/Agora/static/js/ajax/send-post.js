...
function createAndAppendPost(response) {
  ...
  let DynamicProf = response.profile_photo
    ? response.profile_photo
    : "../static/images/default-avatar-profile-picture-male-icon.png";
  ...
  postContainer.innerHTML = `
    ...
    response.firstname
                ...
                <div class="UPC-content-grid ${
                  response.photos.length >= 2 ? "three-photos" : ""
                }" style="${
    response.photos.length === 0 ? "display: none;" : ""
  }">
                    ${response.photos
                      .map(
                        (photo) =>
                          `<div class="UPC-content ${
                            response.photos.length === 1 ? "single-photo" : ""
                          }">
                            <img class="lazy" src="${
                              photo.link
                            }/tr:q-90,tr:w-450,bl-30,q-90,h-450" data-src="${
                            photo.link
                          }/tr:q-90,tr:w-450,h-450">
                        </div>`
                      )
                      .join("")}
                </div>
                <div class="UPC-content-Bottom">
                    ...
                    <div class="UPCB-Reacts">
                        <div class="Reacts">
                            <div class="GlowReact-Div" data-PostIDD="${
                              response.post_id
                            }" data-AccID="${response.accId}">
                                ${
                                  response.has_liked
                            ...
                            <div class="React-Div Comment-Btn-Show" data-PostID="${
                              response.post_id
                            }">
                        ...
                        <div class="React-Counts">
                            <div class="glow-count">
                                <p>${response.glows_count} ${
    response.glows_count > 1 ? "glows" : "glow"
  }</p>
                            </div>
                            <div class="Comments">
                                <p>${response.comment_count} ${
    response.comment_count > 1 ? "comments" : "comment"
  }</p>
                            </div>
                        </div>
                    </div>
